...
(defn route->path
  ...
  ([bidi-routes route-name params error-callback]
   (try
     (path->hash (path-for bidi-routes route-name params))
     (catch js/Error e
...
(defn navigate!
  ...
  ([router bidi-routes route-name params]
   (.setToken router (token->path (route->path bidi-routes route-name params))))
  ([router bidi-routes route-name params error-callback]
   (.setToken router (token->path (route->path bidi-routes route-name params error-callback)))))
